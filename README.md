...
- **2026-09-14 — Subida de archivos en "Recursos clave":** botón real de subida (antes solo se podían pegar links). Los archivos van a un bucket público de Supabase Storage (`briefs-recursos`, tope 15 MB por archivo) y su link se suma automáticamente a `recursos_clave` junto con los enlaces tipeados a mano. El panel ahora muestra ese campo con los links clickeables. Bucket y políticas creados en producción; probado end-to-end (subida real + fetch del link público). Supabase marcó de entrada una advertencia de seguridad ("Clients can list all files in this bucket") por la política de SELECT que había agregado — se sacó esa política (innecesaria: el bucket público ya sirve los links directos sin ella) y se verificó que el acceso a archivos existentes no se rompió.
